...
Clone the git repositories for alphanode and gitian and then checkout the alphanode version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the alphanode repository
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/alphanode/.git/
...
./bin/gbuild --commit alphanode=${COMMIT} --url alphanode=${URL} ../alphanode/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/alphanode-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/alphanode-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/alphanode-build.assert
```
...
[alphanode/gitian.sigs](https://github.com/alphanode-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
